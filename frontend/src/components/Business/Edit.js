...
class Edit extends Component {
  ...
  onChangeHandler = () => {
    ...
    this.getVideoCover(file, 1.5).then((cover) => {
      ...
      // form_data.append("advert_slug", this.state.slug);

      Axios.post("http://localhost:8000/api/updateBusinessVideo", {
        advert_title: document.getElementById("title").value,
        advert_category: document.getElementById("category").value,
        advert_budget: document.getElementById("budget").value,
        advert_file: document.getElementById("video_file").files[0],
        advert_image: file,
        advert_slug: this.state.slug,
      })
        .then((response) => {
  ...
  render() {
    // console.log("values: " + this.state.info);
    return (
      //   <div>
